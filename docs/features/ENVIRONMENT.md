...
reportar. Ya se ha hecho tres veces en este repo con buen resultado. **Y donde se pone importa:** un `.html` de arnés colocado en `public/` **no** lleva el preambulo de `@vitejs/plugin-react` y la pagina muere con «can't detect preamble»; tiene que ir en la **raiz** del repositorio para que Vite lo transforme. Le costo tres turnos a un revisor el 2026-09-25.

**Y al medir anchos, el panel miente.** La pestaña del navegador emula **568 px**, así que medir «a 375» sobre ella da números de otro ancho. La forma que funciona es meter la pantalla en un `iframe` del ancho exacto — y ojo: un contenedor `flex` **encoge el `iframe`** ignorando su atributo `width`, lo que ya costó una medición perdida. Esta semana dos defectos de ancho solo aparecieron midiendo, y uno únicamente a **760 px**, porque a 375 la fila ya envolvía: mira los dos anchos siempre. Y una duda que ya apareció una vez: **el `iframe` también vale para las media queries** —es su propio viewport—, comprobado el 2026-09-24 midiendo lo mismo de las dos formas (`iframe` a 768 px y viewport emulado a 768 px dan la misma cifra). Lo que no vale es medir sobre la pestaña sin emular, que son 568.
...
| Linter | `pnpm lint` | **14 errores / 0 warnings**, preexistentes. `eslint-plugin-react-hooks` 7 cierra a la vez las dos salidas clásicas para «decidir algo una sola vez cuando llegan los datos»: leer un `useRef` durante el render (`Cannot access refs during render`) y `setState` dentro de un `useEffect` (`set-state-in-effect`). Lo que pasa limpio es el `setState` condicional **durante el render** con el pestillo dentro del propio estado, como en `CreateVidaCategoryStep.tsx` (FEAT-017 tajada 3). |
| Tests | `pnpm test` | **2 fallos de 2353** (`SearchSelect` ×2, preexistentes). **Los sábados son 3**: `HabitPanel.test.tsx:566` no congela el reloj y cuenta domingos en una ventana de 90 días (12 semanas + 6 días: un día de la semana sale 12 veces y cuál depende de la fecha); comprobado con `TZ=Pacific/Kiritimati`, donde pasa 25/25. Congelar ese reloj es otro expediente. A veces salen **3**: `IconPicker.test.tsx > normalizes selection to stored name bell` es **flaky en la corrida completa** —el esqueleto de carga sigue en el DOM, el archivo tarda ~10 s— y **pasa 6/6 corriéndolo solo**. Visto en FEAT-014 tajada 2. Si aparece, córrelo aislado antes de culpar a tu cambio. |
| Paquete | `pnpm build` | chunk inicial **1.157,77 kB** (pasó del megabyte tras FEAT-005; el troceado es deuda propia) + `app-icons` 620 kB perezoso + `IconPicker` 4,6 kB. **Y el CSS: 281,63 kB.** Está aquí porque el 2026-09-22 un bloque de comentario sin cerrar en un `.module.scss` se comio cuatro reglas —el semaforo entero y tres estilos ya aceptados— y **nada lo cazo**: no es TypeScript, el linter no mira SCSS, vitest no compila CSS y el build paso en verde. La unica senal fue que el CSS bajo de 274,18 a 273,61 kB. Si esta cifra **baja** sin que hayas borrado estilos a proposito, busca un comentario abierto sin cerrar antes de seguir. Bajo una vez a proposito, de 274,32 a 274,19 el 2026-09-23: la regla `.line` de `VidaGoalArc.module.scss`, al quitar la frase de debajo del arco (FEAT-019 tajada 5). Asi es como se comprueba que una bajada es legitima: compilar el SCSS de `HEAD` y el del arbol con `sass --style=compressed` y **comparar la lista de selectores**, no el tamano — un comentario sin cerrar se lleva las reglas siguientes y ahi se ve. |
...
**No todo el código vive en `src/`.** Desde FEAT-021 hay código de build propio en `vite/` —`cache-shape.ts`, que decide cuándo caduca la caché que el navegador guarda en `localStorage`— y **sus tests corren con vitest desde fuera de `src/`**. Si tocas la capa de datos y quieres saber por qué se ha tirado (o no) la caché de todo el mundo, el rastro es `buster` → `src/app/providers/query-persist.ts` → `vite/cache-shape.ts`, y la cabecera de ese fichero lo explica.
...
| Qué | Comando | Línea base hoy (2026-09-22) |
|---|---|---|
| Tipos | `npx tsc --noEmit` | limpio, exit 0 — **pero NO es puerta para los tests**: el `tsconfig.json` de ese repo excluye `tests/`, asi que los tipos salian limpios con tres suites que no compilaban. La unica puerta que lo caza es `npm test`. No metas `tests` en ese tsconfig (arrastra `rootDir` y el `outDir` del build); si algun dia hace falta, un `tsconfig.tests.json` aparte. |
| Tests | `npm test` (jest) | **1 fallo de 646**, y **1 suite de 55 en rojo** (2026-09-24, tras FEAT-024). Ese fallo **no es un test desfasado: es un defecto real del API** — `walletExpenseUpdate` llega al servicio como `updateExpense(undefined, userId, undefined)` porque `expenseUpdateSchema` no declara `id` ni `input` y `withValidation` sustituye los args por lo que devuelve el esquema. **Editar un gasto no funciona en produccion.** Se deja rojo a proposito hasta que tenga su feature; si algun dia sale verde sin que nadie lo arregle, sospecha. Antes de FEAT-024 esto decia «3 fallos de 614, 6 suites rojas»: **tres** suites no compilaban (no cuatro), y no por lo que yo tenia escrito — `habit-streak` si por `HabitStreakFields`, pero `sleep-follow-up-sync` y `standup` por `UserSettings` ganando los campos de la noche y por `Activity.isWorkout`. `habit-streak` no arrastraba a nadie. |
...
- **Para el revisor:** el constructor corre `graphify update .` al terminar, así que cuando llegas **el grafo ya es el estado posterior al cambio**. Sirve para «quién llama ahora»; para «quién dependía antes» (un símbolo borrado, una firma cambiada) usa `git grep` sobre `HEAD` o `git archive`, no el grafo. Visto en FEAT-010 y FEAT-012.
