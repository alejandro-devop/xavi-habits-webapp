...
describe('ColorPicker', () => {
  ...
  it('pone el núcleo primero y los extendidos detrás', () => {
    ...
    expect(labels.slice(0, 6)).toEqual(CORE_COLORS.map((color) => color.label))
  })

  it('marca el color que ya tiene, aunque llegue en mayúsculas', () => {
  ...
  it('se recorre con las flechas del teclado', async () => {
    const user = userEvent.setup()
    const onChange = vi.fn()
    render(<ColorPicker value={PALETTE_COLORS[0].hex} onChange={onChange} />)

    screen.getByRole('radio', { name: 'Menta' }).focus()
    await user.keyboard('{ArrowRight}')

    expect(onChange).toHaveBeenCalledWith(PALETTE_COLORS[1].hex)
  })
  ...
  it('enseña el color de fuera de la paleta que ya tenía', () => {
    ...
    expect(screen.getAllByRole('radio')).toHaveLength(PALETTE_COLORS.length + 1)
  })
})
